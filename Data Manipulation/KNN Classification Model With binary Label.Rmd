...
```
The probabilitities come on a form that ROCR does not accept so we need to invert them for the -1 class and rescale them.
...
plot ROC
```{r}
plot(pred_knn, avg= "threshold", colorize=T, lwd=3, main="KNN Binary Label ROC Curve")
```
